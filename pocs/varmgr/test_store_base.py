...
def test_get_source_names(varmgr):
    ...
    # Test all sources
    expected_all = [
        "app_cli", "app_env", "app_secret", "project_cli", "project_env",
        "project_secret", "stack_cli", "stack_env", "app_config",
        "project_config", "app_defaults", "project_defaults",
        "stack_defaults", "stack_secret", "stack_config",
    ]
    ...
    # Test project scope
    expected_project = [
        "project_cli", "project_env", "project_defaults",
        "app_cli", "app_env", "app_defaults"
    ]
    assert varmgr.get_source_names(scope="scope_project") == expected_project

    # Test stack scope
    expected_stack = [
        "stack_cli", "stack_env", "stack_defaults",
        "project_cli", "project_env", "project_defaults",
        "app_cli", "app_env", "app_defaults"
    ]
...
def test_get_values_fallback(varmgr):
    ...
    assert varmgr.get_value("fallback_project", scope="scope_project") == "fallback_dataset2"

    # Test stack scope fallback
    assert varmgr.get_value("fallback_stack", scope="scope_stack") == "fallback_dataset3"
...
def test_get_var_names(varmgr):
    ...
    expected_names = {
        "name", "path", "test_override", "fallback_app",
        "description", "options", "fallback_project", "fallback_stack"
    }
...
def test_reference_missing_source():
    ...
    with pytest.raises(VarMgrAppError):
        varmgr.set_scopes({
            "test_scope": ["non_existent_source"]
        })
...
def test_circular_scope_reference():
    ...
    varmgr = StoreManager()
    varmgr.add_sources([
        Source("source1", level=100),
        Source("source2", level=200)
    ])
    
    with pytest.raises(VarMgrAppError):
        varmgr.set_scopes({
            "scope1": ["source1", "scope2"],
            "scope2": ["source2", "scope1"]
        })
